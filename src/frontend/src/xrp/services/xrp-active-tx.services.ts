...
/**
 * Record ids currently being driven by a live `sendXrp` in this tab.
 *
 * The poller and an open send modal would otherwise resolve the same record
 * concurrently: the modal's own confirmation loop polls the identical hash, and
 * whichever wrote a terminal status first would win, because terminal statuses
 * are immutable on the backend. This is the hazard the ICPSwap migration hit —
 * a poller closing a record the modal is still driving.
 *
 * Correctness does not rest on this set. Both paths establish expiry the same
 * strict way, so either one's conclusion is sound on its own; what the set buys
 * is that the record's transitions are written by the code that owns the send
 * while it still owns it, instead of two writers racing. A record belonging to a
 * dead session or another tab is not in the set, which is exactly why the poller
 * still resolves it.
 */
const ownedByLiveSend = new Set<string>();

export const claimXrpActiveUserTransaction = (id: string): void => {
	ownedByLiveSend.add(id);
};

export const releaseXrpActiveUserTransaction = (id: string): void => {
	ownedByLiveSend.delete(id);
};
