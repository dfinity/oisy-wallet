...
/**
 * The EVM tokens added for the 1Sec integration (BOB/CHAT/GLDT/ICP on Ethereum,
...
describe('1Sec EVM tokens are metadata-only', () => {
	...
	it('does not suggest any 1Sec EVM ICP token', () => {
		const suggestedIds = new Set(ERC20_SUGGESTED_TOKENS.map(({ id }) => id));

		expect(suggestedIds.has(ICP_ETH.id)).toBeFalsy();
		expect(suggestedIds.has(ICP_ARBITRUM.id)).toBeFalsy();
		expect(suggestedIds.has(ICP_BASE.id)).toBeFalsy();
	});
});
