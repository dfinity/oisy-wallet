...
describe('calculateEthFee', () => {
	...
	describe('ERC20 token', () => {
		...
		it('should use token amount for ERC20 gas estimation when specified', async () => {
			vi.mocked(erc20Utils.isTokenErc20).mockReturnValue(true);

			const erc20TokenWithAmount = {
				...baseToken,
				standard: 'erc20',
				address: '0x1234567890123456789012345678901234567890',
				amount: '100'
	...
	describe('network configuration', () => {
		it('should call getEthFeeDataWithProvider with correct parameters', async () => {
			vi.mocked(ethUtils.isSupportedEthTokenId).mockReturnValue(true);
		...
		it('should pass correct chainId from token network', async () => {
			vi.mocked(ethUtils.isSupportedEthTokenId).mockReturnValue(true);

			const customChainId = 137n;
			const customNetwork = { ...network, chainId: customChainId };
			const tokenWithCustomNetwork = { ...baseToken, network: customNetwork };

			await calculateEthFee({
				userAddress: fromAddr,
				token: tokenWithCustomNetwork
			});
