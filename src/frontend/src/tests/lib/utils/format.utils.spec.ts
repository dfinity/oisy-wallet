...
describe('format.utils', () => {
	describe('formatToken', () => {
		...
		it('should format zero with default parameters', () => {
			expect(formatToken({ value: 0n })).toBe('0');
		});

		it('should format zero with specified displayDecimals', () => {
			expect(formatToken({ value: 0n, displayDecimals: 2 })).toBe('0');
		});

		it('should format zero with trailing zeros', () => {
			expect(formatToken({ value: 0n, trailingZeros: true })).toBe('0.0000');
		});

		it('should format zero with specified displayDecimals and trailing zeros', () => {
			expect(formatToken({ value: 0n, displayDecimals: 2, trailingZeros: true })).toBe('0.00');
		});
