...
describe('derived-memo.utils', () => {
	describe('derivedMemo', () => {
		...
		it('should allow an equality function that always returns false', () => {
			const source = writable(1);
			const store = derivedMemo(
				source,
				($source) => $source % 2,
		...
		it('should not synchronously emit an uninitialised value when the source does not emit immediately', () => {
			let subscriber: ((value: number) => void) | undefined;
		...
		it('should emit the first value once the source emits later', () => {
			...
			const store = derivedMemo(source, ($source) => $source.toUpperCase(), Object.is);
		...
		it('should immediately emit the memoised value to a later subscriber after a delayed first emission', () => {
			let subscriber: ((value: number) => void) | undefined;
			...
			expect(firstValues).toEqual([11]);

			const unsubscribeSecond = store.subscribe((value) => {
				secondValues.push(value);
			});

			expect(secondValues).toEqual([11]);

			unsubscribeFirst();
			unsubscribeSecond();
		});

		it('should not emit to a subscriber that unsubscribes before the source emits its first value', () => {
			let subscriber: ((value: number) => void) | undefined;
			...
			const store = derivedMemo(source, ($source) => $source * 5, Object.is);
		...
		it('should not emit duplicate values when the delayed first and second source emissions derive to equal values', () => {
			let subscriber: ((value: number) => void) | undefined;
