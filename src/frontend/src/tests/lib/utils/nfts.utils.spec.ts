...
import { getLoadedNftsByNetworkAndToken } from '$lib/utils/nfts.utils';
...
describe('nfts.utils', () => {
	...
	describe('getLoadedNftsByNetworkAndToken', () => {
		it('should return nfts for a given list of tokens and networks', () => {
			...
			const result: NftsByNetwork = getLoadedNftsByNetworkAndToken({
		...
		it('should return empty lists for tokens for which no nfts were provided', () => {
			const result = getLoadedNftsByNetworkAndToken({ tokens: erc721Tokens, loadedNfts: [] });
		...
		it('should return an empty map if no tokens and no nfts are provided', () => {
			const result = getLoadedNftsByNetworkAndToken({ tokens: [], loadedNfts: [] });
