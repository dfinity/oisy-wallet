	...
	describe('parseEthereumUri', () => {
		...
		describe('valid URIs', () => {
			it('should parse complete URI with all required fields', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1?value=1000`);
			...
			it('should parse URI with different chainId', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@137?value=1000`);

				expect(result?.chainId).toBe(137);
			});

			it('should parse URI with large value', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1?value=1000000000000000000`);

				expect(result?.value).toBe(1000000000000000000n);
			});

			it('should parse URI with zero value', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1?value=0`);

				expect(result?.value).toBe(0n);
			});

			it('should parse URI with chainId zero', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@0?value=1000`);
			...
			it('should preserve address case', () => {
				...
				const result = parseEthereumUri(`ethereum:${mixedCaseAddress}@1?value=1000`);

				expect(result?.address).toBe(mixedCaseAddress);
			});
		});

		describe('missing required fields', () => {
			it('should return undefined when chainId is missing', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}?value=1000`);

				expect(result).toBeUndefined();
			});

			it('should return undefined when value is missing', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1`);

				expect(result).toBeUndefined();
			});

			it('should return undefined when both chainId and value are missing', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}`);

				expect(result).toBeUndefined();
			});

			it('should return undefined when address is missing', () => {
				const result = parseEthereumUri('ethereum:@1?value=1000');

				expect(result).toBeUndefined();
			});

			it('should return undefined when all fields are missing', () => {
				const result = parseEthereumUri('ethereum:');
		...
		describe('invalid scheme', () => {
			it('should return undefined for non-ethereum scheme', () => {
				const result = parseEthereumUri('bitcoin:0x123@1?value=1000');

				expect(result).toBeUndefined();
			});

			it('should return undefined for http scheme', () => {
				const result = parseEthereumUri('http://example.com');
			...
			it('should return undefined for scheme with whitespace', () => {
				const result = parseEthereumUri('ethereum:   ');
		...
		describe('invalid address format', () => {
			it('should return undefined for invalid address', () => {
				const result = parseEthereumUri('ethereum:invalid-address@1?value=1000');
			...
			it('should return undefined for address without 0x prefix', () => {
				const result = parseEthereumUri(
					'ethereum:742d35Cc6634C0532925a3b844Bc9e7595f0bEb@1?value=1000'
				);
			...
			it('should return undefined for short address', () => {
				const result = parseEthereumUri('ethereum:0x123@1?value=1000');

				expect(result).toBeUndefined();
			});

			it('should return undefined for long address', () => {
				const result = parseEthereumUri(
					'ethereum:0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb123@1?value=1000'
				);

				expect(result).toBeUndefined();
			});

			it('should return undefined for address with invalid characters', () => {
				const result = parseEthereumUri(
					'ethereum:0x742d35Cc6634C0532925a3b844Bc9e7595f0bEG@1?value=1000'
				);
		...
		describe('invalid chainId', () => {
			...
			it('should return undefined for non-numeric chainId', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@abc?value=1000`);
		...
		describe('invalid value', () => {
			it('should return undefined for negative value', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1?value=-100`);

				expect(result).toBeUndefined();
			});

			it('should return undefined for non-numeric value', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1?value=abc`);

				expect(result).toBeUndefined();
			});

			it('should return undefined for decimal value', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1?value=1.5`);

				expect(result).toBeUndefined();
			});

			it('should return undefined for empty value', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1?value=`);
		...
		describe('edge cases', () => {
			it('should handle URI with extra query parameters', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1?value=1000&gas=21000`);
			...
			it('should handle URI with value as first parameter', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1?value=1000&other=param`);

				expect(result?.value).toBe(1000n);
			});

			it('should handle URI with very large chainId', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@999999999?value=1000`);

				expect(result?.chainId).toBe(999999999);
			});

			it('should handle URI with very large value', () => {
				const largeValue = '999999999999999999999999999999';
				const result = parseEthereumUri(`ethereum:${validAddress}@1?value=${largeValue}`);

				expect(result?.value).toBe(BigInt(largeValue));
			});

			it('should handle URI with chainId having leading zeros', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@001?value=1000`);

				expect(result?.chainId).toBe(1);
			});

			it('should handle URI with value having leading zeros', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1?value=00100`);
		...
		describe('malformed URIs', () => {
			it('should return undefined for malformed query string', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1?`);
			...
			it('should not throw on unexpected input', () => {
				expect(() => parseEthereumUri('ethereum:???@@@')).not.toThrow();
			});

			it('should not throw on special characters', () => {
				expect(() => parseEthereumUri('ethereum:$%^&*()@1?value=1000')).not.toThrow();
			});
		});

		describe('whitespace handling', () => {
			it('should handle URI with whitespace in address', () => {
				const result = parseEthereumUri(`ethereum: ${validAddress} @1?value=1000`);

				expect(result).toBeUndefined();
			});

			it('should handle URI with whitespace around value', () => {
				const result = parseEthereumUri(`ethereum:${validAddress}@1?value= 1000 `);
