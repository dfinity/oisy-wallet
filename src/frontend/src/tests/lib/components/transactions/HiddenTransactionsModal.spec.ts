...
describe('HiddenTransactionsModal', () => {
	...
	describe('when the transactions list is empty', () => {
		...
		it('should render the placeholder', () => {
			...
			expect(
				getByText(replaceOisyPlaceholders(get(i18n).transactions.text.hidden_receive))
			).toBeInTheDocument();
		});
	});
	...
	describe('when the transactions list is not empty', () => {
		...
		it('should render the transactions list with all the receive micro transactions', () => {
			...
			const transactionGroupComponents = Array.from(
				container.querySelectorAll('div[data-tid*="all-transactions-date-group"]')
			);
