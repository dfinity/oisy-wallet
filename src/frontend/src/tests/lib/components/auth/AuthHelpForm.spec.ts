...
describe('AuthHelpForm', () => {
    ...
    it('should render auth help form content', () => {
        ...
        const lostIdentityButton: HTMLButtonElement | null = container.querySelector(lostIdentityButtonSelector);
        expect(lostIdentityButton).toBeInTheDocument();
        expect(getByText(get(i18n).auth.help.text.lost_identity)).toBeInTheDocument();

        const securityButton: HTMLButtonElement | null = container.querySelector(securityButtonSelector);
        expect(securityButton).toBeInTheDocument();
        expect(getByText(get(i18n).auth.help.text.security)).toBeInTheDocument();

        const gotConfusedButton: HTMLButtonElement | null = container.querySelector(gotConfusedButtonSelector);
