...
describe('AuthHelpOtherForm', () => {
    ...
    it('should render auth help other form content',() => {
        ...
        const introductionAnchor: HTMLAnchorElement | null = container.querySelector(introductionAnchorSelector)
        expect(introductionAnchor).toBeInTheDocument();
        expect(introductionAnchor?.href).toBe(OISY_DOCS_URL + '/');
        expect(getByText(replaceOisyPlaceholders(get(i18n).auth.help.text.other_introduction))).toBeInTheDocument();
        ...
        expect(getByText(replaceOisyPlaceholders(get(i18n).auth.help.text.other_docs))).toBeInTheDocument();

        const privateKeyAnchor: HTMLAnchorElement | null = container.querySelector(privateKeyAnchorSelector)
        expect(privateKeyAnchor).toBeInTheDocument();
        expect(privateKeyAnchor?.href).toBe(OISY_FAQ_URL);
        expect(getByText(replaceOisyPlaceholders(get(i18n).auth.help.text.other_private_key))).toBeInTheDocument();

        const assetControlAnchor: HTMLAnchorElement | null = container.querySelector(assetControlAnchorSelector)
        expect(assetControlAnchor).toBeInTheDocument();
        expect(assetControlAnchor?.href).toBe(OISY_ACCESS_CONTROL_URL);
        expect(getByText(replaceOisyPlaceholders(get(i18n).auth.help.text.other_asset_control))).toBeInTheDocument();
