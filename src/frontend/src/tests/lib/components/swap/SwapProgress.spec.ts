...
describe('SwapProgress', () => {
	...
	describe('sendWithApprovalOnly', () => {
		...
		it('renders the approve step without either signing step', () => {
			const { container } = render(SwapProgress, {
				props: { sendWithApprovalOnly: true }
			});
		...
		it('highlights the approve step it renders', () => {
			const { container } = render(SwapProgress, {
				props: { sendWithApprovalOnly: true, swapProgressStep: ProgressStepsSwap.APPROVE }
			});
		...
		// by id. The props serve different wizards, so this is an invariant.
		it('adds no second approve row when sendWithApproval already contributes one', () => {
			const { container } = render(SwapProgress, {
				props: { sendWithApproval: true, sendWithApprovalOnly: true }
			});
