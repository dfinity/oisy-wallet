...
import { collectionMetadata as fetchCollectionMetadata } from '$icp/api/icrc7.api';
...
import { mapIcrc7CollectionMetadata } from '$icp/utils/icrc7.utils';
...
> => {
	...
	const builtIn = ICRC7_BUILTIN_TOKENS_INDEXED.get(canisterId);

	if (nonNullish(builtIn)) {
		const { symbol, ...rest } = builtIn;
	...
	const entries = await fetchCollectionMetadata({ canisterId, certified, identity });
