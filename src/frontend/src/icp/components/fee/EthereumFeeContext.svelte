<script lang="ts">
	...
	$: ckEthConvert = isConvertCkEthToEth({ token, networkId });

	let ckErc20Convert = false;
	$: ckErc20Convert = isConvertCkErc20ToErc20({ token, networkId });
	...
	$: maxTransactionFeeCkEth = nonNullish(token.id)
	...
	$: tokenCkEth = $icrcTokens
		...
		.find((tokenCkEth) => isTokenIcrcTestnet(tokenCkEth ?? {}) === isTokenIcrcTestnet(token));
	...
	const loadFee = async () => {
		...
		const load = async () => await loadEip1559TransactionPrice(token);
