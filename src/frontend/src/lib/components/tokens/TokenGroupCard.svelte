<script lang="ts">
	...
	const totalBalance = tokenGroup.tokens.reduce((sum, token: TokenUi) => sum + (token.balance ?? 0), 0);

	const totalBalanceUsd = tokenGroup.tokens.reduce((sum, token: TokenUi) => sum + (token.usdBalance ?? 0), 0);
</script>
