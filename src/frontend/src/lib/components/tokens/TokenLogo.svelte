...
<div class="relative">
	...
	{#if badge?.type === 'tokenCount' && badge.count > 0}
		<span
			class="border-tertiary absolute -right-2.5 bottom-0 flex h-6 w-6 items-center justify-center rounded-full border-[0.5px] bg-white text-sm font-semibold text-black"
	...
	{:else if badge?.type === 'icon'}
		...
		<div
			class="bg-brand-tertiary absolute -right-1 -bottom-1 h-6 w-6 items-center justify-center rounded-full p-1 text-white"
