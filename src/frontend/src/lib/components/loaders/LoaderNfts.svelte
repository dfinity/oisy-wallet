	...
	const onLoad = async () => {
		...
		for(const token of $enabledNonFungibleTokens) {
			...
			if (token.standard === 'erc721') {
				...
				const newNftIds = inventory.filter((tokenId) => isNullish(findNft({nfts: $nftStore ?? [], token, tokenId})))
				const removedNfts = findRemovedNftIds({nfts: $nftStore ?? [], token, nftIds: inventory})

				await loadNftIdsOfToken({infuraProvider: infuraErc721Provider, token, tokenIds: newNftIds, walletAddress: address})
			}
			if (token.standard === 'erc1155') {
				const alchemyProvider = alchemyProviders(token.network.id);

				const inventory = await alchemyProvider.getNftIdsForOwner({address, contractAddress: token.address})

				const newNftIds = inventory.filter((ownedNft) => isNullish(findNft({nfts: $nftStore ?? [], token, tokenId: ownedNft.id})))
					.map((ownedNft) => ownedNft.id)
				const removedNfts = findRemovedNftIds({nfts: $nftStore ?? [], token, nftIds: inventory.map((ownedNft) => ownedNft.id)})

				const updatedNfts = ($nftStore ?? []).filter((nft) => {
				...
				if(newNftIds.length > 0) {
					...
					await loadNftIdsOfToken({infuraProvider: infuraErc1155Provider, token, tokenIds: newNftIds, walletAddress: address})
				}
