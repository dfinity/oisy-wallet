<script lang="ts">
	...
	let mainCardMessageParams = $derived(
		nonNullish(mainCard.token) &&
			(nonNullish(mainCard.network) || (secondaryCards ?? []).length < 1)
			? {
					messageText: replacePlaceholders($i18n.ai_assistant.text.send_token_on_network_message, {
						$token: getTokenDisplaySymbol(mainCard.token),
						$network: (mainCard.network ?? mainCard.token.network).name
					})
				}
			: nonNullish(mainCard.token)
				? {
...
<div class="flex w-full flex-col rounded-lg border-brand-subtle-20 bg-brand-subtle-10">
	...
	>
		{#snippet cardDescription()}
			{#if isNullish(mainCard.network) && nonNullish(secondaryCards) && secondaryCards.length > 1}
				{#each [...new Set(secondaryCards.map(({ network: { name } }) => name))] as network, index (network)}
					{#if index !== 0}
						<Divider />
					{/if}{network}
				{/each}
			{:else if nonNullish(mainCard.token)}
				{mainCard.token.network.name}
			{/if}
		{/snippet}
	</AiAssistantShowBalanceToolCard>
