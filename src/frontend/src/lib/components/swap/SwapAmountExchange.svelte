<script lang="ts">
	...
	$: amountUSD = mode === 'usd' && nonNullish(amount) && nonNullish(exchangeRate) ? Number(amount) * exchangeRate : 0;

	let formattedTokenAmount: string | undefined;
	$: formattedTokenAmount =  nonNullish(token) ? mode === 'token' && nonNullish(amount) ? Number(amount).toString() + ' ' + token.symbol : 0 + ' ' + token.symbol: '0';
</script>
