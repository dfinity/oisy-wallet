...
{#if nonNullish(valueDifference)}
	<span
		class="underline"
		class:text-success={valueDifference > SWAP_VALUE_DIFFERENCE_WARNING_VALUE}
		class:text-warning={valueDifference <= SWAP_VALUE_DIFFERENCE_WARNING_VALUE &&
			valueDifference > SWAP_VALUE_DIFFERENCE_ERROR_VALUE}
		class:text-error={valueDifference <= SWAP_VALUE_DIFFERENCE_ERROR_VALUE}
	>
