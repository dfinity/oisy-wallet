<script lang="ts">
	...
	let { currentSlide, index, totalSlides, onClick }: Props = $props();
...
<button
	class="{`${isActive ? 'bg-primary-inverted w-7' : 'bg-disabled w-4'} mr-1 h-1.5 transition-all duration-300 ease-linear last:mr-0`}}"
