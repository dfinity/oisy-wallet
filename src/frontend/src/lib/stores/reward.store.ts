...
export const initRewardEligibilityContext = (store: RewardEligibilityStore): RewardEligibilityContext => {
	return {
		store,
		getCampaignEligibility: (rewardId: string) =>
			derived(store, ($store) =>
				$store?.campaignEligibilities?.find(
					({ campaignId }) => campaignId === rewardId
				)
			)
	};
};
