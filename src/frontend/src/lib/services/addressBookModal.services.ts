...
import { loading, currentContact, currentContactId, currentAddressIndex, qrCodeAddress, previousStepName } from '$lib/stores/addressBookModal.store';
...
export type AddressBookDeps = {
  ...
  track: {
    createSuccess: string; createError: string;
    updateSuccess: string; updateError: string;
    deleteSuccess: string; deleteError: string;
    avatarUpdateSuccess: string; avatarUpdateError: string;
    avatarDeleteSuccess: string; avatarDeleteError: string;
  };
};

const withState = <T extends (...a: any[]) => Promise<any>>(fn: T) => async (...args: Parameters<T>) => {
  loading.set(true);
...
export function makeController(deps: AddressBookDeps) {
  const callCreateContact = withState(wrapCallWith({
  ...
  const callUpdateContact = withState(wrapCallWith({
  ...
  const callDeleteContact = withState(wrapCallWith({
