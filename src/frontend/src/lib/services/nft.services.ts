...
import { transferExtV2 } from '$icp/services/nft-send.services';
import { loadNfts as loadExtNfts } from '$icp/services/nft.services';
import type { IcNonFungibleToken } from '$icp/types/nft';
import { isTokenExtV2, isTokenExtV2CustomToken } from '$icp/utils/ext.utils';
...
export const sendNft = async ({
...
} & Pick<TransferParams, 'to'> &
	(
		| (Pick<TransferParams, 'from' | 'maxFeePerGas' | 'maxPriorityFeePerGas'> & {
				gas: bigint;
		  })
		| {}
	)) => {
	...
	if (isNetworkIdEthereum(networkId) || isNetworkIdEvm(networkId)) {
		...
		const { from, to, gas, maxFeePerGas, maxPriorityFeePerGas } = rest;
