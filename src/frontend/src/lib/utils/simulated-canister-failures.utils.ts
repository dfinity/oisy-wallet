 ...
 *
 * Nothing here is gated on the environment: the harness branch carries an unconditional banner
 * saying what it is, and the selection below is empty until a tester fills it in — so the code is
 * inert by default in any build, which is a stronger guarantee than an env flag.
 *
...
const QA_LOGGING_ENABLED = !import.meta.env.VITEST;
...
export const qaLog = (...args: unknown[]): void => {
	...
	console.log(`[Testing harness:${QA_CONTEXT}]`, ...args);
};
...
const initSimulatedFailuresStore = () => {
	const { subscribe, set } = writable<SimulatedFailures>(readStoredFailures());
