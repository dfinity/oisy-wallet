...
export function isSplTokenToggleDisabled(token: SolanaUserToken): boolean {
	return nonNullish(token) && token.category === 'default' && token.standard === 'solana';
}

export function isSplTokenToggleEnabled(token: SolanaUserToken): boolean {
	return !isSplTokenToggleDisabled(token);
}
