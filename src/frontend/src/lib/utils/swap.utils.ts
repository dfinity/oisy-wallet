...
export const getLiquidityFees = (transactions: SwapAmountsTxReply[], tokens: Token[]): ProviderFee[] => {
...
export const getNetworkFee = (transactions: SwapAmountsTxReply[], tokens: Token[]): ProviderFee | undefined => {
    if (transactions.length === 0) {
        return undefined;
    }
...
const findToken = (tokens: Token[], symbol: string): Token | undefined => tokens.find((token) => token.symbol === symbol)