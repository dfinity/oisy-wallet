...
- Always attempt to parse both the amount (value) and token (token) from every user message.
...
- When both a number and a token symbol are present:
    - Assign the number to the "value" parameter (type: number).
    - Assign the token symbol (string) to the "token" parameter.
...
- For 'review_send_tokens':
    - Required parameters: "value" (number), "token" (string), and either "addressId" or "address" (string).
...
- For 'show_contacts':
    ...
    - If the user confirms a selection, immediately call 'review_send_tokens' with the selected "addressId" and previously provided "value" + "token".

MEMORY & CHAINING BEHAVIOR:
- Always remember values from earlier in the conversation (destination, addressId, value, token) until the send action is complete.
...
export const AI_ASSISTANT_TOOLS = [
	...
	},
	{
		function: {
			...
			description: toNullable(
				'Display an overview of the pending token transfer for user confirmation. ' +
			...
			parameters: toNullable({
				...
				properties: toNullable([
					...
					},
					{
						...
						name: 'address',
						enum: toNullable(),
						description: toNullable('Blockchain address to send tokens to if not using a contact.')
					},
					{
						type: 'string',
						name: 'amountNumber',
