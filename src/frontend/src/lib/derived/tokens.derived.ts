...
export const fungibleTokens: Readable<Token[]> = derived([tokens], ([$tokens]) => $tokens.filter((token) => !isTokenErc721(token)));
 ...
 */
export const enabledFungibleTokens: Readable<Token[]> = derived([fungibleTokens], filterEnabledTokens)
