...
import type { Token, TokenWithFinancials } from '$lib/types/token';
...
export const enabledNetworkTokensWithFinancials: Readable<TokenWithFinancials[]> = derived(
	[enabledNetworkTokens, balancesStore, exchanges],
	([$enabledNetworkTokens, $balancesStore, $exchanges]) =>
		$enabledNetworkTokens.map((token) => {
			return {
				...token,
				usdValue: nonNullish($exchanges?.[token.id]?.usd)
					? usdValue({
							token,
