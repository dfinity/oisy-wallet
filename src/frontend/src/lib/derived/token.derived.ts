...
import type { OptionTokenId, OptionTokenStandard, TokenCategory, TokenUi } from '$lib/types/token';
 ...
 */
export const tokenWithFallback: Readable<TokenUi> = derived(
	[token],
	([$token]) => $token ?? (DEFAULT_ETHEREUM_TOKEN as TokenUi)
);
