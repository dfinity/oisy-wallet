...
// Schnorr (Ed25519) schema used by the signer for Solana, mirrored from the frontend's offline
// derivation (`src/frontend/src/lib/ic-pub-key/src/cli.ts`, `deriveSolAddress`).
...
/// signing call). The path mirrors the signer's: `[SCHEMA_SOL, principal, "SOL", "mainnet"]`.
