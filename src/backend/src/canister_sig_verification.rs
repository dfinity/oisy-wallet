// Inlined and patched from `ic-signature-verification` v0.2.0 (Aug 2024).
//
// The published crate only looks up canister ranges at the legacy path
//   `subnet/<subnet_id>/canister_ranges`
// which no longer exists on non-root subnets after IC spec v0.50.0 (Oct 2025).
// The new sharded path is `canister_ranges/<subnet_id>`.
//
// This module adds a fallback so both paths are tried, keeping compatibility
// with older (local dfx) and current (mainnet) certificate formats.
//
...
fn lookup_canister_ranges<'a>(cert: &'a Certificate, subnet_id: &[u8]) -> Result<&'a [u8], String> {
    ...
    Err("canister_ranges-entry not found".to_string())
}
...
fn verify_delegation(
    ...
    // Look up canister ranges: try sharded path first (IC spec v0.50.0+),
    // then fall back to the legacy path for older environments (e.g. local dfx).
    let canister_range = lookup_canister_ranges(&cert, delegation.subnet_id.as_ref())?;
