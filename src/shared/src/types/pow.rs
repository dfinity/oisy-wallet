...
// The average time (in milliseconds) that the system aims for solving each Proof-of-Work (PoW) challenge.
// If auto-adjustment of difficulty is enabled (`DIFFICULTY_AUTO_ADJUSTMENT = true`), the difficulty level
// is adjusted dynamically after each solved challenge so that the average solving time approaches this target.
// A well-chosen TARGET_DURATION_MS ensures that challenges are neither too easy nor too difficult
// to solve, trying to find the balance between system security and usability.
...
// The defined value of EXPIRY_DURATION_MS must always be greater than the value of TARGET_DURATION_MS,
// ensuring that challenges have sufficient time to be solved before expiration.
// By default, EXPIRY_DURATION_MS is set to twice the TARGET_DURATION_MS to provide a reasonable time
// limit for solving challenges.
...
// The number of execution cycles granted to the system for each unit of difficulty in a Proof-of-Work (PoW) challenge.
// A higher difficulty level results in more cycles being granted, since solving the challenge
// requires additional computational resources. This constant directly determines the relationship
// between difficulty and the number of cycles allocated for signer operations.
// For example, with `CYCLES_PER_DIFFICULTY = 10_000`, a challenge with a difficulty of `1_000_000`
// will grant `10_000 * 1_000_000 = 10,000,000,000` cycles.
// It is important to find the correct balance here (also taking the cycles used to execute the PoW code itself into account)
