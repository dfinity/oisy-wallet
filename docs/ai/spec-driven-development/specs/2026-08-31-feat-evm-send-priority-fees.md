...
- **Feature:** Let the user choose Slow / Standard / Fast for an ETH or ERC-20 send and, per [section 10](#10-walletconnect), for a WalletConnect transaction request, and show a fee that reflects the choice
...
destructures **only `medium`** and discards the rest. Slow / Standard / Fast maps 1:1 onto
...
| Fee state    | Selected tier lives in `SendContext`, defaults to Standard                                        |
...
discretion, e.g. `EthFeePriority` with `SLOW` / `STANDARD` / `FAST`) and keep the Gas API's
...
> choices with different trade-offs. It is **Standard** everywhere now, in the enum, the i18n keys
> and the copy, which is also what the public gas trackers call this tier. The Figma frames above
> still read Normal and have not been re-exported.
...
- no tier UI yet; still pinned to Standard
...
- tier state in `SendContext`, default Standard
...
- the default is Standard on open
...
2. ~~**Do the tiers separate on L2s?**~~ **Answered, and the premise was wrong. See section 9.**
   ...
   `high` carry the same tip, so Standard and Fast quote an identical fee. This is not an L1/L2
...
5. **Persistence.** Does the choice reset to Standard on every send, or persist for the session or
...
- Under **Transaction priority**: _"The choice is offered on the send flow only: WalletConnect
  requests, swaps, conversions and staking still use the standard speed."_
