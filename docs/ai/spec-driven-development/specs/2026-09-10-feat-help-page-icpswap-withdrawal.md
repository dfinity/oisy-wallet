...
| `scan`           | `icpswap_withdrawal` | the scan button completes                                        | `executing` → `success` / `error` / `cancel` (superseded) | `event_key: balances_found`, `event_value`: count of withdrawable rows; `source_detail`: number of pools scanned; `result_error_type` on failure                          |
