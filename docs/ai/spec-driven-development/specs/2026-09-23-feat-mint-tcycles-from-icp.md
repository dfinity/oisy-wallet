...
4. A row whose transfer outcome was never observed (the tab died during the transfer call) is resolved by **looking the deposit up** in the user's ICP history: the transfer to the CMC deposit account with the `MINT` memo and the transfer creation timestamp stored in the row. It is never resolved by sending again. If no deposit is found once the transfer can no longer land, the row is deleted: nothing moved. What bounds that moment is the modal itself, which sends the transfer only within a minute of the timestamp it stored; the call then expires within minutes.
