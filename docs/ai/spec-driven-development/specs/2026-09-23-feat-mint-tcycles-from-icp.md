...
3. The global poller drives a row with a block index: `notify_mint_cycles` settles it as minted, refunded or failed (§3.6), while `Processing` or an unreachable CMC keeps it in flight, asked again about once a minute rather than on every poll.
4. A row whose transfer outcome was never observed (the tab died during the transfer call) is resolved by **looking the deposit up** in the ICP history of the user's CMC deposit account: the transfer to it with the `MINT` memo and the transfer creation timestamp stored in the row. That account only holds mint deposits and their burns and refunds, so the search stays short however busy the wallet is. It is never resolved by sending again. If no deposit is found once the transfer can no longer land, the row is closed as failed, not sent: nothing moved, but the user started this mint and may be watching it in Active transactions, where a row that vanished would leave them guessing. What bounds that moment is the modal itself, which sends the transfer only within a minute of the timestamp it stored; the call then expires within minutes.
