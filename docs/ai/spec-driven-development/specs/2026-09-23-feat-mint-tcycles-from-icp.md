...
2. A row whose transfer the ICP ledger refused is deleted rather than closed as failed: nothing moved, and a failed row would have the user look for ICP that never left the wallet (the OISY Trade precedent).
