...
| 3     | `feat/frontend/xrp-rpc-client`          | Phase 2    | **This PR.** XRPL JSON-RPC client — `loadXrpBalance` (`account_info`) via `$xrp/api/xrpl.api.ts` + `$xrp/providers/xrp-rpc.providers.ts`, `XRP_RPC_HTTP_URL_MAINNET` env, `docs/ai/integrations/xrpl.md`, mocked-fetch unit tests. No CSP change (covered by the existing `https:` wildcard).                                               |
