...
| 6     | `feat/frontend/xrp-send-orchestration`  | Phase 5    | `sendXrp` pipeline (fetch sequence + destination existence + ledger index → build → sign → submit → confirm; the reviewed fee is passed IN, not fetched), the sequence/fee/ledger/tx API fetches, the locally derived transaction id, reserve checks on both sides of the payment via `getXrpMaxAmount`/`getXrpReserveDrops`, `isInvalidDestinationXrp`, the typed send errors, reserve/fee/ledger constants, `ProgressStepsSendXrp` + send analytics. Tested logic; no UI.                                                                                                   |
| 7     | `feat/frontend/xrp-send-ui`             | Phase 6    | Send flow UI — wizard + form + reserve-aware amount + destination + **destination-tag input** + review + fee context/display, wired to `sendXrp`; send-modal (`SendWizard`) and destination-step (`SendDestinationWizardStep`) routing, `sendXrpDestinationTag` in the send context. XRP address-book contacts are out of scope (need a backend address type).                                                                                                                                                                                                                |
| 8     | `feat/frontend/xrp-transactions`        | Phase 7    | Transaction history from XRPL `account_tx`: `loadXrpTransactions` (marker-paginated) + `mapXrpTransaction` (successful native-XRP Payments only, send/receive relative to the wallet, drops value, sender-paid fee, ripple-epoch → Unix timestamp, destination tag), `xrpTransactionsStore`, the scheduler + listener loading balance + history together, the `xrpTransactions` derived + `component: 'xrp'` cross-chain aggregation, and the list/row/detail-modal UI (destination-tag row + XRPScan links). v1 shows the latest page — no infinite-scroll or backend cache. |
| 9     | `feat/backend/xrp-network-settings`     | —          | **Backend prerequisite, branched off `main` (not stacked).** `NetworkSettingsFor::XrpMainnet` so a user's per-network enable/disable choice can be persisted (without it, any user with saved network settings gets no XRP key, `isUserNetworkEnabled` falls back to false and XRP silently disappears), plus `TokenId::XrpNativeMainnet` and the CoinGecko `ripple` mapping so the backend can price XRP. This is the one place the integration has to touch `backend.did`.                                                                                                  |
| 10    | `feat/frontend/xrp-enable`              | Phase 9    | **This PR.** Remove the temporary `XRP_MAINNET_DISABLED_OVERRIDE` (so `VITE_XRP_MAINNET_DISABLED` governs XRP like every other chain), map XRP in `networkIdToKey` / `keyToNetworkId`, exchange-rate/price lookup, `PRODUCT.md` + `docs/ai/integrations/xrpl.md` updates, and the suite-wide network/token expectations that only change once XRP is enabled.                                                                                                                                                                                                                 |
