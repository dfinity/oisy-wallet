...
| 6     | `feat/frontend/xrp-send-orchestration`  | Phase 5    | **This PR.** `sendXrp` pipeline (fetch sequence + destination existence + ledger index → build → sign → submit → confirm; the reviewed fee is passed IN, not fetched), the sequence/fee/ledger/tx API fetches, the locally derived transaction id, reserve checks on both sides of the payment via `getXrpMaxAmount`/`getXrpReserveDrops`, `isInvalidDestinationXrp`, the typed send errors, reserve/fee/ledger constants, `ProgressStepsSendXrp` + send analytics. Tested logic; no UI.                          |
| 7     | `feat/frontend/xrp-send-ui`             | Phase 6    | Send flow UI — destination-tag input, reserve-aware max-amount, review/progress wizard, send-modal + destination-step routing, fee context/display — wired to `sendXrp`                                                                                                                                                                                                                                                                                                                                           |
