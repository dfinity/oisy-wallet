...
A user can get there today, but only by copying their address out of Receive, working out which explorer serves that chain, and pasting it in — once per chain. That detour is exactly the kind of thing that becomes a support message, which is the Help page's inclusion test.
...
**Each link is labelled with the network's own name and logo.** A column of plain text links reads as a wall; the logos make the list scannable, and the names come from the `Network` objects rather than from i18n, because they are proper nouns.
