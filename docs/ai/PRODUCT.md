...
Signed-in users can enter a **Plug wallet seed phrase** on a dedicated page (`/plug-import/`), reached from an **Import from Plug** entry in Settings, and see what that wallet holds on the networks OISY supports. The entry point is deliberately in Settings rather than the primary navigation: it is a one-off migration tool, and keeping it behind sign-in preserves the rule that OISY only ever asks for a seed phrase inside the signed-in app — a standalone page that asks for one is an ideal phishing template.

Only Plug's **Internet Computer identity** is derived from the seed phrase. Plug's Bitcoin, EVM and Solana addresses are **chain-key (threshold) keys held by Plug's own Chain Fusion helper canister**, addressed by the resulting principal — the same architecture OISY uses with the chain-fusion-signer, rooted in a different canister. OISY can therefore reproduce every Plug address offline from the phrase alone, with no calls to Plug, but **cannot sign** for Bitcoin, EVM or Solana: those keys exist only inside Plug's canister.

- **Nothing is persisted.** The phrase lives in the page's own state for as long as the tab is open and is never written to `localStorage`, `sessionStorage`, IndexedDB, the URL, or the user profile. Reloading the page discards it and the user starts over. The input is also kept out of reach of autofill and password managers. OISY does **not** become a custodian of the phrase or of any key derived from it.
- **Multiple accounts.** Plug numbers its accounts in order, each a different principal with its own set of addresses, so the user chooses how many accounts to check (up to ten). Each is listed separately with its principal and its per-network balances.
- **Balances shown.** Every enabled fungible token the import can read: ICP and ICRC by principal, ERC20 by contract on each supported EVM network, SPL by associated token account, and the native coin on Bitcoin, each EVM network, and Solana. Balances of zero are hidden — the page exists to show what there is to move — while a balance that could not be loaded is shown as **unavailable**, so an unreachable provider never looks like an empty account. The page explains what **unavailable** means, because some ledgers are permanently unreadable: DIP20 tokens expose no ICRC balance method and are skipped entirely, and a few ICRC ledgers have been uninstalled and can never answer.
...
- **View only, for now.** The page does not move anything. Sweeping ICP / ICRC balances into the user's own OISY account, and handling the remaining chains, are follow-ups.
