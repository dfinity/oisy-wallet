...
Closing an account pays out **every lamport it holds** to the destination the instruction names, and that destination is stated. It is usually the user's own wallet, and the line says the balance came back; when it is not, the line says where it went instead and names the account, because a close that hands a funded wrapped SOL account to somebody else is otherwise indistinguishable from one that returns it. The amount is every lamport the account holds rather than its rent alone — for a wrapped SOL account, the rent plus whatever was wrapped, since there the wrapped amount is the lamports. A token balance of any other mint has to be zero for the close to succeed, so it is never what leaves this way. The cost the review quotes follows the same distinction, taken to the end of the chain: a close that pays the user back reduces what the transaction costs them, and one whose balance ends up with somebody else does not — including when it reaches them through another account of the user's that is itself closed onward. A close hands on everything the account holds by then — whatever it already held, what the message paid into it, and what an earlier close in the same message paid into it — so the amount stated at each one is the balance as it stood there. Only closes later in the message are followed when deciding where a balance ends up; an unsigned message can name any sequence, including one that points back at itself.
