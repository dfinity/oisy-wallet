...
An Ethereum or EVM send lets the user pick how fast it should confirm: **slow**, **normal** or **fast**. This is currently limited to local and staging builds; beta and production keep the previous single-speed form. Normal is the default and the recommendation, and the choice lasts for that one send rather than being remembered. Each option is priced against the same transaction, so the amounts differ only by the tip the sender is willing to add, which is the part of the fee they actually control. Picking a different speed re-prices from the sample already in hand rather than asking the network again, so the quoted fee updates immediately. Whatever is chosen is what gets signed. On a small screen the options open in a sheet; on a large one they expand in place. Where the network reports no choice, the row does not appear and the send behaves as it did before. The choice is offered on the send flow only: WalletConnect requests, swaps, conversions and staking still use the normal speed.
...
OISY ships a **proof-of-concept** native app shell for Android and iOS, built with Capacitor from the same SvelteKit codebase (`mobile/`, `capacitor.config.ts` inside). The apps are not published to any store; CI (`mobile-build.yml`) produces a debug APK and an unsigned iOS build as workflow artifacts.

**Login preserves the web identity.** Inside the app, sign-in does not use the in-WebView popup flow (WebAuthn is unavailable there). Instead, the app generates an Ed25519 session key and opens the **system browser** on the `/mobile-auth` bridge page served by the canonical web origin. The bridge validates the request (exact-match redirect-URI allowlist, hex session key), runs the standard Internet Identity sign-in for that session public key, and hands the resulting delegation chain back to the app via the `oisy://auth-callback` deep link — in the URL fragment only. Because the delegation is requested from the canonical origin, the derived principal — and therefore the user's wallet, addresses, and assets — is **identical** to the web wallet. The app validates that the chain is bound to its own session key before accepting it.

Deliberate POC limitations (excluded on purpose, see `docs/ai/spec-driven-development/specs/2026-07-10-feat-mobile-app-poc.md`, including its threat model): custom URL scheme instead of verified Universal/App Links (interceptable by another installed app — the decisive reason the POC must not ship to real users as-is), session key in WebView storage rather than Keychain/Keystore, a 1-day mobile session TTL without a biometric app lock, unscoped delegations (no `targets`), and no store distribution. The web experience is unchanged; the bridge page is inert unless opened with valid parameters.
