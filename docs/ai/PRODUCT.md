...
What a transaction did is read from the network once and then kept in the browser, so opening Activity, opening a token's page and reloading the app do not fetch the same transaction again. Only transactions the network has finalized are kept, the newest ones per network, and signing out clears them along with the rest of the session.
