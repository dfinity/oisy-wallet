...
The details of the newest Solana transactions are kept in the browser once read from the network, so reloading the app, or enabling a token whose history the wallet already loaded, does not fetch them again. Only transactions the network has finalized are kept, the newest 200 per network, and they are kept per wallet rather than shared between the wallets of one browser. Nothing is kept before sign-in, and signing out removes them along with the rest of the session.
