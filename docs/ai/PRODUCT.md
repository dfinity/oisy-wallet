...
- **Balances shown.** ICP and ICRC token balances are itemized, plus the native balance on Bitcoin, each supported EVM network, and Solana. Balances of zero are hidden — the page exists to show what there is to move — while a balance that could not be loaded is shown as **unavailable**, so an unreachable provider never looks like an empty account.
