...
The details of the newest Solana transactions are kept in the browser once read from the network, so reloading the app, or enabling a token whose history the wallet already loaded, does not fetch them again. Only transactions the network has finalized are kept, the newest 200 per network, and signing out clears them along with the rest of the session. This cache never holds up what it exists to speed up: if the browser's storage stops answering, OISY stops using it for the rest of that page load and reads what it needs from the network instead, so balances and history still load. Signing out completes whether or not the cache could be cleared.
