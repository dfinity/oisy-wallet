...
Deliberate POC limitations (excluded on purpose, see `docs/ai/spec-driven-development/specs/2026-07-10-feat-mobile-app-poc.md`): custom URL scheme instead of verified Universal/App Links, session key in WebView storage rather than Keychain/Keystore, a 30-day mobile session TTL without a biometric app lock, and no store distribution. The web experience is unchanged; the bridge page is inert unless opened with valid parameters.
