...
ICPSwap also tracks a second, **mistransferred** balance, for tokens transferred to a pool canister without a matching deposit call. That is deliberately **not** covered, because OISY cannot create one: ICPSwap is only offered for ICRC-2 source tokens, and the swap refuses any other before transferring anything. ICPSwap routes such tokens elsewhere anyway — it answers a mistransfer query for a pool's own trading pair with "use deposit and withdraw instead".
