...
> the frontend is gated by a hardcoded boolean flag (e.g.
> `*_PROVIDER_ENABLED` / `*_<FEATURE>_ENABLED` in `$env/rest/*.env.ts`) so it can
> be blocked in code during an incident, and a feature with multiple providers
